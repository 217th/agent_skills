...
- [ ] **Минимальный контракт логов**
  - Зафиксирован бэкенд и формат логов (предпочтительно структурированные JSON‑логи) и ожидания по рантайму (stdout/stderr → backend).
  - Список обязательных полей для корреляции  
    (минимум: `service`, `env`, один/несколько идентификаторов для корреляции, `event`, `severity`, `error.code` при ошибках).
  - Перечень ключевых событий, которые **обязаны** логироваться (ingest, ключевые шаги обработки, завершение операции).
  - Есть хотя бы 1 пример JSON‑события (если применяются структурированные логи).
...
- [ ] **Расширенный контракт логов и observability**
  - Полный список **обязательных и опциональных** полей для логов (включая корреляцию, попытки/ретраи, тайминги, ошибки, ссылки на артефакты).
  - Полный список **обязательных и опциональных** событий (каталог/таксономия событий; стабильные имена в snake_case).
  - Есть примеры JSON‑событий для happy‑path, error‑path и ожидаемых no‑op’ов.
  - Есть “наложение” событий на high‑level flow выполнения (типичный порядок событий + ожидаемые no‑op сценарии).
  - Зафиксированы требования к безопасности: что запрещено логировать, и какие безопасные альтернативы используются (хэши/размеры/URI вместо payload’ов).
